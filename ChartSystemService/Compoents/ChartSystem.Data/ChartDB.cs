...
namespace ChartSystem.Data
{
    public class ChartDB : DbContext
    {
        ...
        public ChartDB(string connString) : // 数据库链接字符串
            base(connString)
        {
            strConn = connString;
            Database.SetInitializer<ChartDB>(null);//设置为空，防止自动检查和生成
            base.Database.Log = (info) => Debug.WriteLine(info);
        }

        public ChartDB(string connString, string logUserName, string logAdditionalInfo) : // 数据库链接字符串
            base(connString)
        {
            strConn = connString;
            Database.SetInitializer<ChartDB>(null);//设置为空，防止自动检查和生成
        ...
        public void AddLogger(string info)
        {
            if (info != "\r\n" && (!info.Contains("Sys_EventLog")))
            {
                ...
                else
                {
                    ...
                    {
                        //增加新日志
                        using (ChartDB db = new ChartDB(strConn))
                        {
                            try
                            {
                                //保存日志到数据库或其他地方
    ...
    public class ChartDB<T> : ChartDB where T : class
    {
        public ChartDB(string connString) : // 数据库链接字符串
            base(connString)
        {
            Database.SetInitializer<ChartDB<T>>(null);//设置为空，防止自动检查和生成
        }
